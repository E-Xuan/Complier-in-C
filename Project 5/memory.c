...
void storeRec(char* iden, int index, int value) {
	...
	if (index + 2 <= callStack[fp]->rValues[location][0] + 1) {
		callStack[fp]->rValues[location][index + 2] = value;
	} else {
...
int recallRec(char* iden, int index) {
	...
	if (index + 2 <= callStack[fp]->rValues[location][0] + 1) {
		return callStack[fp]->rValues[location][index + 2];
	} else {
...
void record(char* lhs, char* rhs) {
	...
	if (locRhs != -1) {
		...
		printf("ERROR: Recond does not exist\n");
		exit(0);
	}
...
void allocateRecord(char* iden, int size) {
	...
	callStack[fp]->rValues[location] = calloc(size + 2, sizeof(int));
	callStack[fp]->rValues[location][0] = size;
	callStack[fp]->rValues[location][1] = 1;
...
void mainFree() {
	for(int i = 0; i < callStack[0]->rLen; i++) {
		if (callStack[0]->rValues[i][1] != 0) {
			callStack[0]->rValues[i][1]--;
